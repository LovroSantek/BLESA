...
static void _button_debouncer(TimerHandle_t timer);
 ...
 */
static void _btn_isr(void *p_arg);
...
button_gpio_t *button_gpio_create(uint8_t pin, bool b_is_active_on_high_level, btn_gpio_pressed_t p_button_pressed_cb)
{
    ...
    p_btn->timer = xTimerCreate("DEBOUNCE TIMER", pdMS_TO_TICKS(DEBOUNCE_TIME_MS), pdFALSE, (void *) p_btn, _button_debouncer);
    if (NULL == p_btn->timer)
    {
        return NULL;
    }
...
static esp_err_t _button_create(button_gpio_t *p_btn)
{
    ...
    if(ESP_OK == esp_err)
    {
        ...
        esp_err = gpio_isr_handler_add(p_btn->pin, _btn_isr, (void *)p_btn);
    }
...
static void _button_debouncer(TimerHandle_t timer)
{
    button_gpio_t *p_btn_gpio = (button_gpio_t *)pvTimerGetTimerID(timer);

    if (_is_button_pressed(p_btn_gpio))
    {
        p_btn_gpio->p_btn_pressed_cb(p_btn_gpio);
    }
}

//---------------------------- INTERRUPT HANDLERS -----------------------------
static void IRAM_ATTR _btn_isr(void *p_arg)
{
    button_gpio_t *p_btn_gpio = (button_gpio_t *)p_arg;

    if (pdFAIL == xTimerStartFromISR(p_btn_gpio->timer, NULL))
    {
        ESP_LOGE("BUTTON: ", "Error while starting a timer for button 1.");
    }
}